...
# /mnt/build/cargo-target on the mounted internal-HDD ext4 filesystem, never on the
# SSD or an external USB. Defaults are isolated per canonical checkout path (D20),
# so worktrees cannot reuse each other's workspace artifacts. This script never
# copies, moves, removes, or changes an old target tree.
#
...
case $config_resolved in
  "$checkout"/*) ;;
  *) die "Cargo config resolves outside the checkout: $config_resolved" ;;
esac
...
emit_config() {
  printf '%s\n' \
    ...
    '# New targets use the mounted internal-HDD ext4; never the SSD or external USB.' \

...
//! types mirror only the fields the shell reads or writes, spelled as
//! `p1-module-protocol` spells them.
...
impl Input {
    pub fn raw(&self) -> p1_shell_guest::RawInput<'_> {
        match self {
            Self::Json(raw) => p1_shell_guest::RawInput::Json(raw),
            Self::Text(raw) => p1_shell_guest::RawInput::Text(raw),
        }
    }
}
...
/// The tool-outcome text of a guest outcome.
pub fn outcome(outcome: &p1_shell_guest::Outcome) -> String {
    text(&Outcome {
        status: match outcome.status {
            p1_shell_guest::Status::Ok => "ok",
            p1_shell_guest::Status::Error => "error",
            p1_shell_guest::Status::Cancelled => "cancelled",
        },

...
## Iris TUI migration (#91, slice #93) — 2026-09-24 22:00, Iris lead

Branch `task/iris-tui-migration` (base `5a4d126`, `main` merged in). Slice #93 adapts the
pinned donor sanitizer (`iris-agent@5b04a1ad`, `src/ui/textengine.rs`) as
`p1-tui/src/text.rs` behind `Band::render`; frozen `tests/band_sanitize.rs` unchanged.
wf6's CSI-abort finding is fixed with a test; its separator finding was reversed after the
PR #102 review (wf3): the SLAB row rule keys the separator on the right segment list and
is ported 1:1. Lead checks: full guarded gate GREEN on the merged tree before that change,
focused p1-tui suites green after it; evidence in
`docs/design/iris-tui-migration-evidence.md`. Not merged: needs the full gate on the final
tree, a re-review and the p1 lead's OK (this file and `lib.rs` are shared seams). The live
TUI restart stays the owner's decision.
Next: freeze the port scope into slices for the owner's ~50-worker workflow (read-only
survey wf2 is evidence only), write and dry-run its Rhai script; plan rev 1 in

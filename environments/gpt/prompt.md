...
# Finishing
- When the task is done, verify it with a command through `{{tool:shell}}` that would fail if it
  were wrong, then call `{{tool:finish}}` with status "done" and name the exact commands you ran.

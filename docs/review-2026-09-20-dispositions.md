...
|---|---|---|---|---|
| D-A | Shared workspace ownership weakened: `seams.md §8` requires conflicting parallel writes to be serialized or isolated; `delegation.md` made it "the model's responsibility" | Yes. The detailed spec said so openly, but it is a weakening of the baseline, not a refinement | OPEN — next work item, before any concurrent dogfooding. Start with serialization of potentially conflicting writers. Per-path grants are not claimed to constrain shell commands without an execution boundary | plan step 1; GitHub issue |
| D-B | Changed-route resume warns, drops foreign reasoning replay and proceeds; `seams.md §3` requires translation or explicit rejection | Yes | OPEN — reject a resume whose route origin differs until a route/history compatibility policy exists; decided in an ADR | plan step 4; GitHub issue |
...
| D-D | Resume is not restartable orchestration: child sessions, worker ids and retained repair state do not survive the process | Yes — deferred capability, not a defect of ordinary resume | OPEN — make it explicit to the user and the model on resume (children of the earlier process are gone), and record it in an ADR. Persistent child lifecycle only after longer sessions succeed | plan step 4; GitHub issue |

...
|---|---|---|---|---|
| D-A | Shared workspace ownership weakened: `seams.md §8` requires conflicting parallel writes to be serialized or isolated; `delegation.md` made it "the model's responsibility" | Yes. The detailed spec said so openly, but it is a weakening of the baseline, not a refinement | OPEN — next work item, before any concurrent dogfooding. Start with serialization of potentially conflicting writers. Per-path grants are not claimed to constrain shell commands without an execution boundary | plan step 1; issue #1 |
| D-B | Changed-route resume warns, drops foreign reasoning replay and proceeds; `seams.md §3` requires translation or explicit rejection | Yes | OPEN — reject a resume whose route origin differs until a route/history compatibility policy exists; decided in an ADR | plan step 4; issue #2 |

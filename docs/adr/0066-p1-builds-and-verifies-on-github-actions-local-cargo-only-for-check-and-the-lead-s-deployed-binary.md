...
p1 builds and verifies on GitHub Actions. A push to any `task/**` branch runs `scripts/gate.sh` and `cargo build -p p1-host` (debug, as deployed) in `.github/workflows/build.yml` and uploads the `p1` binary, its sha256 and the gate log as the `p1-build` artifact. `scripts/ci-build.sh [<branch>] [--no-download] [--wait-only]` pushes that branch, waits for the run of exactly that commit (as `scripts/push-main.sh` does for main), prints the summary, downloads the artifact to `ci-artifacts/<sha>/` and exits 0 (success, sha256 verified), 1 (the run failed or was cancelled) or 2 (usage or tooling error). Local cargo is limited to `cargo check -p <crate>` and the lead's deployed-binary rebuild; a full local gate is run only when Actions cannot be used. One script serves both repositories through `CI_TRIGGER=push|dispatch` (default `push`): `dispatch` is for a repository whose workflow does not run on push, and it pushes the branch, reuses a `build.yml` run for exactly that commit that is still queued or running (or is green), and otherwise starts one with `gh workflow run build.yml --ref <branch>` and waits for that `workflow_dispatch` run.
...
- `python3 scripts/test_ci_build.py` (30 tests, stubbed `git`/`gh`/`sleep`, no network) covers argument parsing, the `main`/detached-HEAD refusals the 0/1/2 mapping and both trigger modes: failure and cancellation exit 1 with the failed-step log tail, unverified sha exits 1, push/`gh`/download failures and a missing run exit 2, an invalid `CI_TRIGGER` exits 2, and dispatch either reuses a queued/running/green run of the commit or starts one and waits for it.

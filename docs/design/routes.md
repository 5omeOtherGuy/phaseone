...
route — `opencode-go-1-subscription` (`OPENCODE_GO_1_API_KEY`, out of credit 2026-09-23),
...
fallback exists) — and environments `zen`, `zen2`, `zen3` pick one as their default, so a workflow
can spread work across accounts. No Zen usage endpoint is established, so these routes probe as

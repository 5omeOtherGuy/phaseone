...
| `textengine.rs::{display_width,cluster_width,cluster_advance,truncate_to_width,wrap_to_width}` | MIT; unicode-segmentation, unicode-width | **Blocked**: p1 width equivalence not yet proven; donor oversized cluster overflow and zero-width policy must not be copied blindly | p1-tui wrap; existing Band/transcript consumers | #94; bounded progress, grapheme and measured-height tests needed |
...
| `textengine.rs` | parser, width, ZWJ, wrap/truncate families | MIT | unicode-segmentation, unicode-width; zwj_probe reference | **Adapted** sanitizer before transcript measurement/wrapping; width, ZWJ and remainder Blocked; p1-tui pure `text` sanitizer, consumed by `render::transcript`; #135 accepted |
...
| `tui/wrap.rs` | styled wrap/truncate, clamps | MIT | ratatui, unicode-segmentation; hyperlink, textengine | Blocked; #94/text foundation follow-up |
| `tui_loop.rs` | run, input/event phases, deferred commands | MIT | anyhow, ratatui-textarea, tokio; cli/config/git/goal/metrics/mimir/nexus/session/signals/tools/wayland | Blocked; host pump equivalence, never whole runtime |

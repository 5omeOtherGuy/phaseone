(() => {
  ...
  function compose(x,y){
    neural.pause();pending={x,y,time:neural.time,mode:neural.mode,screenshot:$('#brain').toDataURL('image/png')};
  ...
  function show(c){neural.pause();neural.setMode(c.mode);neural.draw(c.time);}
  function render(){
    ...
    for(const [i,c] of comments.entries()){
      ...
      jump.textContent=`${i+1} · ${c.time.toFixed(2)} s · ${c.mode}`;jump.onclick=()=>show(c);
      p.textContent=c.text;li.append(jump,p);$('#comments').append(li);

...
"which source" report. Borrow-only stays: no login command, no writes to p1's own store.
...
A login command, writing api keys to p1's store, macOS paths and Keychain, an OS keyring,
key files outside the known stores (the second OpenCode Go account keeps using its variable).

//! SHA-256 of a module's bytes: the digest that is its identity (freeze item 6).
//!
//! Copied from `crates/p1-hook-shadow/src/lib.rs` (`sha256`, with its FIPS 180-4 vectors):
//! the bounded dependency set of this crate has no hash crate, and the digest must be
//! computed over exactly the bytes the loader compiles, so it is computed here.

/// The hex form of a digest, lowercase, as `sha256sum` and the release manifest write it.

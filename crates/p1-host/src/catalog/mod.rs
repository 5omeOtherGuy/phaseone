...
// Declared after `apply_face!`: a `macro_rules!` macro is visible only to the code
// that follows it, and the tool registrations below use it.
mod delegation;
mod modules;
mod providers;
mod tools;
...
pub(crate) use delegation::register_workflow_tools;

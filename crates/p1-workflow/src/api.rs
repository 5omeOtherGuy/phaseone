...
// The value types the decision contract carries (the step envelope and what it is made of)
// live in `api/values.rs`, a file that needs nothing but serde, serde_json and std: the
// workflow-decision component compiles that same file (S6.9), so the contract has one
// source. Re-exported, so every `p1_workflow::api::*` path stays what it was.
mod values;
...
pub trait WorktreeHold: Send + Sync {
    ...
    /// The worktree as the step left it: its `head` read again.
...
pub trait StepRunner: Send + Sync {
    ...
    fn worktree<'a>(
        ...
        Box::pin(async move {
            Err(format!(
                "worktree: {}: this host does not make worktrees",
                request.worktree.as_deref().unwrap_or_default()
            ))
        })
    }
}
...
// `StepEnvelope`, `StepStatus`, `ModelTry` and `MovedOn` are in `api/values.rs`.

// ------------------------------------------------------------------ journal (item 6)

/// The format version of a run's `journal.jsonl` this build writes and reads. The run
/// journal is its own format, not the session journal's: it carries its own version, in
/// the leading `started` record (see [`JournalRecord::Started`]).
pub const WORKFLOW_JOURNAL_VERSION: u32 = 1;

/// The version of a journal whose `started` record carries none: every journal written
/// before the version existed is exactly the version-1 format.
fn journal_written_before_versions() -> u32 {
    1
}

/// One line of a run's `journal.jsonl`, append-only. `Started` is always the first line
/// and carries the journal's format version; `Dispatch` is written BEFORE the worker
/// starts (caps are rebuilt from these on resume, counting every dispatch), `Result`
/// after each step, `Ended` last.
#[derive(Debug, Clone, PartialEq, Serialize, Deserialize)]
#[serde(tag = "kind", rename_all = "snake_case")]
pub enum JournalRecord {
    Started {
        /// The journal's format version ([`WORKFLOW_JOURNAL_VERSION`] when written by this
        /// build). It lives in the leading record rather than on a line of its own so every
        /// line stays one tagged record, which readers of the journal's kinds rely on. A
        /// reader refuses a version it does not know instead of guessing at the records
        /// that follow (`read_journal`); a `started` record without it is version 1.
        #[serde(
            rename = "p1_workflow_journal",
            default = "journal_written_before_versions"
        )]
        journal_version: u32,
        run: RunId,
...
mod tests {
    ...
    #[test]
    fn journals_written_before_worktrees_still_read() {
        let started: JournalRecord = serde_json::from_str(
            r#"{"kind":"started","run":"wf1","script_hash":"00","args":{},"resumed_from":null}"#,
        )
        .unwrap();
        assert!(matches!(started, JournalRecord::Started { base: None, .. }));
        let result: JournalRecord = serde_json::from_str(
            r#"{"kind":"result","call":"c","envelope":{"step":"c","label":null,"status":"done","value":null,"schema":"not_requested","evidence":null,"attempts":1,"worker":null,"needs":null,"error":null,"models":[]}}"#,
        )
        .unwrap();
        assert!(matches!(
            result,
            JournalRecord::Result {
                envelope: StepEnvelope { worktree: None, .. },
                ..
            }
        ));

        let with = JournalRecord::Started {
        ...
        assert_eq!(serde_json::from_str::<JournalRecord>(&json).unwrap(), with);
    }

...
//!
//! [`Transport`] is the only network-shaped type a provider adapter sees; the
//! SSE decoder, status classification and the retrying [`drive`] loop are shared
//! by every adapter. The crate owns no route knowledge: an adapter supplies a
//! request builder and a [`ResponseParser`], and this crate owns the policy that
//! is identical across routes — one credential refresh per request, one shared
//! transient-retry budget, bounded backoff that races cancellation, and the rule
//! that a stream is never retried after any output has been forwarded.
//!
